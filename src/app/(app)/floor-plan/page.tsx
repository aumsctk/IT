...
export default function FloorPlanPage() {
  ...
  const [layers,    setLayers]    = useState({
    ...
    names: true, ports: false, assets: true,
  });
  ...
  function getAssignedTags(excludeSeatId?: string): Set<string> {
    ...
    Object.values(floorData).forEach(fd => {
      fd.seats.forEach(s => {
        if (s.id !== excludeSeatId) (s.asset_tags ?? []).forEach(t => used.add(t));
      });
    });
    return used;
  }
  ...
  function duplicateSeat(seat: Seat) {
    const newSeat: Seat = {
      ...seat, id: uid(),
      label: seat.label + "'",
      x: snap(seat.x + 120), y: snap(seat.y),
      asset_tags: [], employee: undefined,
    };
    setSeats(p => [...p, newSeat]);
    setSelected(newSeat);
  }
  ...
  useEffect(() => {
    function onKeyDown(e: KeyboardEvent) {
      ...
      if (!e.metaKey && !e.ctrlKey && !e.altKey) {
        ...
        if (e.key === "d" || e.key === "D") { setTool("add_seat"); return; }
        if (e.key === "z" || e.key === "Z") { setTool("add_zone"); return; }
        if (e.key === "Escape") { setTool("select"); setSelected(null); return; }
      }
      if (!selected) return;
  ...
  function handleCanvasClick(e: React.MouseEvent) {
    ...
    if (tool === "add_seat") {
      ...
      setSelected(null); setShowLy(false);
    }
  }
  ...
  function handlePrint() {
    ...
    const PX = 3.78;
    const PORT_W = Math.round(170 * PX), PORT_H = Math.round(257 * PX);
    const LAND_W = Math.round(257 * PX), LAND_H = Math.round(170 * PX);
    ...
    const zonesHtml = zones.map(z => `
      <div style="position:absolute;left:${z.x}px;top:${z.y}px;width:${z.w}px;height:${z.h}px;
        background:${z.color}cc;border:1.5px solid ${ZONE_BORDER[z.color] ?? "#94a3b8"};border-radius:10px;box-sizing:border-box;">
        <div style="display:inline-flex;align-items:center;margin:5px 0 0 5px;padding:2px 8px;
          background:rgba(255,255,255,0.8);border-radius:5px;font-size:10px;font-weight:700;color:#475569;">${z.label}</div>
      </div>`).join("");
    ...
    const seatsHtml = seats.map(seat => {
      ...
      const assetRows = tags.slice(0, 3).map(t => {
        ...
        return `<div style="display:flex;align-items:center;gap:3px;font-size:8px;">
          ...
          <span style="font-family:monospace;background:${hr ? "#fef2f2" : "#eef2ff"};color:${hr ? "#dc2626" : "#4338ca"};padding:0 3px;border-radius:3px;">${t}</span>
          ${hr ? "<span>🔧</span>" : ""}
        </div>`;
      }).join("");
      ...
      <div style="position:absolute;left:${seat.x}px;top:${seat.y}px;width:104px;height:104px;
        background:#fff;border:1.5px solid ${SB[seat.status]};border-left:4px solid ${SA[seat.status]};
        border-radius:10px;overflow:hidden;display:flex;flex-direction:column;">
        <div style="padding:4px 6px 3px;display:flex;align-items:center;gap:4px;flex-shrink:0;border-bottom:1px solid #f1f5f9;">
          <div style="width:6px;height:6px;border-radius:50%;background:${SA[seat.status]};flex-shrink:0;"></div>
          <span style="font-size:10px;font-weight:800;color:#0f172a;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1;">${seat.label}</span>
          <span style="font-size:7px;color:#64748b;">${SL_PRINT[seat.status]}</span>
        </div>
        <div style="padding:3px 6px 2px;border-bottom:1px solid #f1f5f9;font-size:9px;color:#374151;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;flex-shrink:0;">
...
<div style="margin-bottom:12px;border-bottom:2px solid #e2e8f0;padding-bottom:8px;display:flex;justify-content:space-between;align-items:flex-end;">
  <div>
    <div style="font-size:16px;font-weight:700;color:#0f172a;">ผังพื้นที่ — ${fl?.label ?? ""}</div>
    <div style="font-size:10px;color:#64748b;">พิมพ์เมื่อ: ${new Date().toLocaleDateString("th-TH", { year: "numeric", month: "long", day: "numeric", hour: "2-digit", minute: "2-digit" })} · A4 ${landscape ? "แนวนอน" : "แนวตั้ง"}</div>
  </div>
  <div style="font-size:10px;color:#64748b;">โต๊ะทั้งหมด ${seats.length} ที่</div>
</div>
<div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:10px;">
  ${(["vacant","occupied","maintenance","broken"] as SeatStatus[]).map(k =>
    `<div style="display:flex;align-items:center;gap:4px;font-size:9px;color:#475569;">
      <div style="width:8px;height:8px;border-radius:50%;background:${SA[k]};"></div>${SL[k].th} (${stats[k]})</div>`
  ).join("")}
</div>
<div style="width:${cW}px;height:${cH}px;transform:scale(${scale.toFixed(4)});transform-origin:top left;
  background:#fff;border:1px solid #e2e8f0;border-radius:10px;position:relative;
  ...
  ${seatsHtml}
    ...
    if (!win) {
      alert(isTh ? "เบราว์เซอร์บล็อกป๊อปอัป — กรุณาอนุญาตป๊อปอัปสำหรับเว็บนี้" : "Popup blocked — please allow popups for this site");
      return;
    }
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      {/* ── Floor Tab Bar ────────────────────────────────────────── */}
      <div
        className="flex items-end gap-0.5 px-3 pt-2 border-b border-white/50 z-20 flex-shrink-0"
        style={GP}
      >
        <div className="flex items-end gap-0.5 flex-1 overflow-x-auto pb-0 [&::-webkit-scrollbar]:hidden" style={{ scrollbarWidth:"none" }}>
          {floors.map(fl => (
            <div key={fl.id} className="group flex items-center flex-shrink-0">
              <button
                onClick={() => { setCurrentId(fl.id); setSelected(null); setSearchResult(null); setSearch(""); }}
                className={`flex items-center gap-1.5 px-4 py-2 text-xs font-medium rounded-t-xl border-t border-x transition-all ${
                  currentId === fl.id
                    ? "bg-white/80 border-white/70 text-indigo-700 -mb-px z-10 shadow-sm"
                    : "bg-transparent border-transparent text-slate-500 hover:text-slate-700 hover:bg-white/30"
                }`}
              >
                <Square size={10} className="opacity-50" />
                {fl.label}
              </button>
              {currentId === fl.id && (
                <button
                  onClick={() => setFloorModal(fl)}
                  className="opacity-0 group-hover:opacity-100 transition-opacity px-1 pb-0.5 text-slate-400 hover:text-indigo-600"
                  title={isTh ? "แก้ไขพื้นที่" : "Edit area"}
                >
                  <Pencil size={9} />
                </button>
              )}
        ...
        <button
          onClick={addFloor}
          className="flex items-center gap-1 px-3 py-2 mb-0 text-xs text-indigo-600 hover:bg-indigo-50/50 rounded-t-xl transition-colors font-medium flex-shrink-0"
        >
        ...
        <div className="flex items-center gap-1.5 pb-2 pl-3 flex-shrink-0">
          {syncErr ? (
            <span className="flex items-center gap-1 text-[10px] text-amber-600 bg-amber-50/80 border border-amber-200/80 rounded-full px-2 py-0.5 font-medium">
              <CloudOff size={10} /> {isTh ? "ซิงก์ไม่สำเร็จ" : "Sync failed"}
            </span>
          ) : (
            <span className="flex items-center gap-1 text-[10px] text-emerald-500 opacity-70">
              <span className="inline-block w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
              {isTh ? "ซิงก์แล้ว" : "Synced"}
            </span>
          )}
        </div>
      </div>
      ...
      {/* ── Toolbar ─────────────────────────────────────────────── */}
      <div
        className="flex items-center gap-2 px-4 py-2 border-b border-white/40 z-10 flex-shrink-0"
        style={{ ...GP, background: "rgba(255,255,255,0.50)" }}
      >
        {/* Tool group */}
        <div className="flex items-center gap-0.5 bg-black/5 rounded-xl p-0.5">
          <TBtn icon={<MousePointer2 size={15} />} label={isTh ? "เลือก/ย้าย  [V]" : "Select/Move  [V]"}
            active={tool === "select"} onClick={() => setTool("select")} />
          <TBtn icon={<Plus size={15} />} label={isTh ? "เพิ่มโต๊ะ  [D]" : "Add Desk  [D]"}
            active={tool === "add_seat"} onClick={() => setTool("add_seat")} />
          <TBtn icon={<Square size={14} strokeWidth={1.5} />} label={isTh ? "เพิ่มโซน  [Z]" : "Add Zone  [Z]"}
            active={tool === "add_zone"} onClick={() => setTool("add_zone")} />
        </div>
        ...
        {tool !== "select" && (
          <span className="flex items-center gap-2 bg-indigo-50 text-indigo-700 border border-indigo-200/80 px-3 py-1 rounded-full text-xs font-medium">
            {tool === "add_seat"
              ? (isTh ? "คลิก canvas เพื่อวางโต๊ะ" : "Click canvas to place desk")
              : (isTh ? "คลิก canvas เพื่อสร้างโซน" : "Click canvas to create zone")}
            <button onClick={() => setTool("select")}><X size={11} /></button>
          </span>
        )}
        ...
        <div className="relative">
          <button
            ...
            className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-xl border transition-all ${
              showLy ? "bg-indigo-50 border-indigo-300/80 text-indigo-700" : "border-white/60 bg-white/40 text-slate-600 hover:bg-white/70"
            }`}
          >
            <Layers size={13} />
            {isTh ? "เลเยอร์" : "Layers"}
            <ChevronDown size={11} className={`transition-transform ${showLy ? "rotate-180" : ""}`} />
          </button>
          {showLy && (
            <div
              className="absolute right-0 top-full mt-1 rounded-2xl shadow-xl p-3 z-50 w-48 space-y-2 border border-white/60"
              style={GP}
            >
              <p className="text-[9px] font-semibold text-slate-400 uppercase tracking-wider">
                {isTh ? "แสดง / ซ่อน" : "Visibility"}
              </p>
              {(Object.entries(layers) as [keyof typeof layers, boolean][]).map(([k, v]) => (
                <label key={String(k)} className="flex items-center justify-between cursor-pointer text-slate-700">
                  <div className="flex items-center gap-2">
                    {(k === "vacant" || k === "occupied" || k === "maintenance" || k === "broken") && (
                      <div className="w-2 h-2 rounded-full" style={{ background: SA[k as SeatStatus] }} />
                    )}
                    {(k === "names" || k === "ports" || k === "assets") && (
                      <div className="w-2 h-2 rounded bg-slate-300" />
                    )}
                    <span className="text-xs">
                      {k === "vacant"      && (isTh ? "ว่าง" : "Vacant")}
                      {k === "occupied"    && (isTh ? "มีคนใช้" : "Occupied")}
                      {k === "maintenance" && (isTh ? "ซ่อมบำรุง" : "Maintenance")}
                      {k === "broken"      && (isTh ? "ชำรุด" : "Broken")}
                      {k === "names"       && (isTh ? "ชื่อพนักงาน" : "Names")}
                      {k === "ports"       && "RJ45 Ports"}
                      {k === "assets"      && (isTh ? "ทรัพย์สิน" : "Assets")}
                    </span>
                  </div>
                  <button
                    onClick={() => setLayers(l => ({ ...l, [k]: !l[k] }))}
                    className={`transition-colors ${v ? "text-indigo-600" : "text-slate-300"}`}
                  >
        ...
        <div className="flex items-center gap-0.5 border border-white/60 bg-white/40 rounded-xl overflow-hidden">
          <button onClick={() => setZoom(z => Math.max(0.3, +(z - 0.1).toFixed(1)))}
            className="px-2 py-1.5 hover:bg-white/70 text-slate-500 transition-colors">
            <ZoomOut size={13} />
          </button>
        ...
        <button onClick={fitToScreen} title={isTh ? "พอดีจอ" : "Fit to screen"}
          className="flex items-center justify-center w-8 h-8 border border-white/60 bg-white/40 text-slate-500 hover:bg-white/70 rounded-xl transition-colors">
          <Maximize2 size={13} />
        </button>
        <button onClick={() => setZoom(1)}
          className="text-xs px-2.5 py-1.5 border border-white/60 bg-white/40 text-slate-600 hover:bg-white/70 rounded-xl transition-colors font-medium">
          100%
        </button>
        <button onClick={handlePrint}
          className="flex items-center gap-1.5 text-xs px-3 py-1.5 bg-white/40 border border-white/60 text-slate-600 hover:bg-white/70 rounded-xl transition-colors font-medium">
          <Printer size={13} /> {isTh ? "พิมพ์" : "Print"}
        </button>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* ── Left Panel ── */}
        <div
          className="w-52 flex flex-col flex-shrink-0 border-r border-white/50 text-xs overflow-hidden"
          style={{ ...GP, background: "rgba(255,255,255,0.55)" }}
        >
          {/* Search */}
          <div className="px-3 py-3 border-b border-white/40">
            <div className="relative">
              ...
              <input
                ...
                placeholder={isTh ? "ค้นหาโต๊ะ / พนักงาน…" : "Search desk / employee…"}
            ...
            {search && (
              <button onClick={() => doSearch(search)}
                className="mt-1.5 w-full text-[11px] bg-indigo-500/10 text-indigo-700 rounded-lg py-1 hover:bg-indigo-500/20 transition-colors font-medium">
                {isTh ? "ค้นหา" : "Search"}
              </button>
            )}
          ...
          {/* Zones list */}
          <div className="px-3 py-3 border-b border-white/40 flex-shrink-0">
            <div className="flex items-center justify-between mb-2">
              <p className="text-[9px] font-semibold text-slate-400 uppercase tracking-wider">
                {isTh ? "โซน" : "Zones"} <span className="text-slate-300 font-normal">({zones.length})</span>
              </p>
              <button onClick={() => setZoneModal("new")}
            ...
            <div className="space-y-0.5 max-h-28 overflow-y-auto">
              {zones.length === 0 && (
                <p className="text-slate-400 text-[11px] italic px-1">{isTh ? "ยังไม่มีโซน" : "No zones yet"}</p>
              )}
              {zones.map(z => (
                <button key={z.id} onClick={() => setZoneModal(z)}
                  className="w-full group flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-white/60 text-left text-slate-700 transition-colors">
                  <span className="w-3 h-3 rounded flex-shrink-0 border" style={{ backgroundColor: z.color, borderColor: ZONE_BORDER[z.color] ?? "#cbd5e1" }} />
                  <span className="truncate text-[11px] flex-1">{z.label}</span>
                  <Pencil size={9} className="opacity-0 group-hover:opacity-60 text-slate-400 flex-shrink-0" />
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="px-3 py-3 flex-1 overflow-y-auto">
            <p className="text-[9px] font-semibold text-slate-400 uppercase tracking-wider mb-3">
              {isTh ? "สรุป" : "Summary"}
            </p>
            <div className="space-y-2.5">
              {(Object.keys(stats) as SeatStatus[]).map(k => (
                <div key={k} className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    <div className="h-1.5 rounded-full bg-current opacity-20" style={{
                      width: `${Math.max(4, seats.length ? (stats[k] / seats.length) * 44 : 0)}px`,
                      color: SA[k],
                      background: SA[k],
                    }} />
              ...
              {seats.length > 0 && (
                <div className="pt-2 border-t border-white/50 flex justify-between text-[10px] text-slate-400">
                  <span>{isTh ? "รวม" : "Total"}</span>
                  <span className="font-semibold text-slate-600">{seats.length} {isTh ? "ที่นั่ง" : "desks"}</span>
                </div>
              )}
            </div>
          </div>
        ...
        >
          {/* Canvas size badge */}
          <div className="absolute bottom-4 left-4 z-10 text-[9px] text-slate-400/70 pointer-events-none select-none font-mono">
            {canvasW} × {canvasH}
          </div>
              ...
              {zones.map(zone => {
                ...
                return (
                  <div key={zone.id} style={{
                    ...
                    {/* Zone drag handle + label */}
                    <div
                      onMouseDown={e => startMoveZone(e, zone)}
                    ...
                    >
                      <Square size={8} style={{ color: bc, opacity: 0.8 }} />
                      {zone.label}
                    </div>
                    {/* Resize handles */}
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: 10,
                      outline: isHit ? "3px solid rgba(245,158,11,0.22)"
                             : isSel ? "3px solid rgba(99,102,241,0.22)" : "none",
                      boxShadow: isSel
                        ? "0 0 0 1px rgba(99,102,241,0.18), 0 4px 16px rgba(99,102,241,0.14)"
                        : "0 1px 6px rgba(15,23,42,0.07), 0 0 0 0.5px rgba(0,0,0,0.03)",
                  ...
                  >
                    ...
                    <div style={{ padding: "5px 6px 3px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
                      ...
                      {layers.ports && seat.rj45_port && (
                        <span style={{ fontSize: 7, fontFamily: "monospace", color: "#64748b", background: "#f1f5f9", padding: "1px 3px", borderRadius: 3, flexShrink: 0 }}>
                          {seat.rj45_port}
                        </span>
                      )}
                    </div>
                    ...
                    <div style={{ padding: "3px 6px 2px", display: "flex", alignItems: "center", gap: 3, flexShrink: 0 }}>
                      {seat.employee ? (
                        <>
                          ...
                          <span style={{ fontSize: 9, color: "#374151", fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                            {layers.names ? seat.employee : "···"}
                          </span>
                        </>
                      ) : (
                          ...
                          {tags.slice(0, 3).map(t => {
                            ...
                            return (
                              <div key={t} style={{ display: "flex", alignItems: "center", gap: 3 }}>
                                <span style={{ fontSize: 10, lineHeight: 1, flexShrink: 0 }}>{categoryIcon(t)}</span>
                                <span style={{
                                  fontSize: 8, fontFamily: "monospace", borderRadius: 3,
                                  color: hr ? "#dc2626" : "#4338ca",
                                  background: hr ? "#fef2f2" : "#eef2ff",
                                  padding: "0 3px", lineHeight: "14px",
                                  maxWidth: 52, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
                                  fontWeight: hr ? 700 : 400, flexShrink: 1,
                                }}>{t}</span>
        ...
        <div
          className="w-64 flex-shrink-0 border-l border-white/50 flex flex-col overflow-hidden"
          style={{ ...GP, background: "rgba(255,255,255,0.58)" }}
        >
          {selected ? (
            <>
              {/* Header */}
              <div className="px-4 py-3 border-b border-white/40 flex items-center justify-between flex-shrink-0">
                ...
                <button onClick={() => setSelected(null)} className="text-slate-400 hover:text-slate-700 flex-shrink-0 ml-2">
                  <X size={15} />
                </button>
              </div>
              ...
              <div className="px-3 py-2.5 border-b border-white/40 flex gap-2 flex-shrink-0">
                ...
                <button
                  onClick={() => duplicateSeat(selected)}
                  title={isTh ? "ทำซ้ำ (⌘D)" : "Duplicate (⌘D)"}
                  className="flex items-center justify-center w-8 h-8 bg-white/70 border border-slate-200/80 text-slate-600 hover:bg-white rounded-xl text-xs transition-colors"
                >
                  <Copy size={12} />
                </button>
                <button
                  ...
                  className="flex items-center justify-center w-8 h-8 border border-red-200/80 text-red-500 hover:bg-red-50/80 rounded-xl text-xs transition-colors"
                >
                  <Trash2 size={12} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    {(["vacant","occupied","maintenance","broken"] as SeatStatus[]).map(st => (
                      <button key={st}
                        onClick={() => {
                          const updated = { ...selected, status: st };
                          setSeats(p => p.map(s => s.id === selected.id ? updated : s));
                          setSelected(updated);
                        }}
                        className={`flex items-center gap-1.5 px-2 py-1.5 rounded-xl border text-[10px] font-semibold transition-all ${
                          selected.status === st ? "shadow-sm" : "border-slate-200/70 text-slate-400 hover:border-slate-300 bg-white/40"
                        }`}
                        style={selected.status === st ? { color: SA[st], background: SC[st], borderColor: SB[st] } : {}}
                      >
                        <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: selected.status === st ? SA[st] : "#cbd5e1" }} />
                        {SL[st][isTh ? "th" : "en"]}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    <PropVal label="X" value={String(selected.x)} mono />
                    <PropVal label="Y" value={String(selected.y)} mono />
                  </div>
                </div>
                ...
                <div>
                  ...
                  {selected.employee ? (
                    <p className="text-xs text-slate-700 flex items-center gap-1.5">
                      <span className="opacity-60">👤</span>{selected.employee}
                    </p>
                  ) : (
                ...
                <div>
                  ...
                  {(selected.asset_tags ?? []).length === 0 ? (
                    <p className="text-xs text-slate-400 italic">{isTh ? "ยังไม่มีทรัพย์สิน" : "None assigned"}</p>
                  ) : (
                    <div className="space-y-1.5">
                      {(selected.asset_tags ?? []).map(t => {
                        const ai = assets.find(a => a.asset_tag === t);
                        const owner = ai?.assigned_to_name;
                        const ownerDiffers = owner && owner !== selected.employee;
                        return (
                          <div key={t} className="rounded-xl border border-indigo-100/80 bg-indigo-50/60 px-2.5 py-2">
                            <div className="flex items-center gap-2">
                              <span className="text-sm">{categoryIcon(t)}</span>
                              <span className="font-mono text-indigo-700 text-[11px] flex-1 truncate">{t}</span>
                              <a href={`/tickets/new?asset_tag=${encodeURIComponent(t)}&desk=${encodeURIComponent(selected.label)}`}
                                className="text-[9px] bg-orange-100 text-orange-700 hover:bg-orange-200 rounded-md px-1.5 py-0.5">🔧</a>
                            </div>
                            {owner && (
                              <p className="mt-0.5 text-[10px]">
                                <span className="text-slate-400">{isTh ? "ผู้ครอบครอง: " : "Owner: "}</span>
                                <span className={ownerDiffers ? "text-amber-600 font-medium" : "text-slate-600"}>
                                  {owner}{ownerDiffers ? " ⚠️" : ""}
                                </span>
                              </p>
                            )}
                          </div>
                ...
                {selected.rj45_port && (
                  <div>
                    <PropLabel>RJ45</PropLabel>
                    <p className="font-mono text-xs text-slate-700">{selected.rj45_port}</p>
                  </div>
                )}
              </div>
            </>
            ...
            <div className="flex-1 flex flex-col items-center justify-center px-6 text-center gap-3">
              <div className="w-12 h-12 rounded-2xl bg-white/70 border border-white/70 flex items-center justify-center shadow-sm">
                <MousePointer2 size={20} className="text-slate-300" />
              </div>
              <div>
                <p className="text-xs font-semibold text-slate-500 mb-1">
                  {isTh ? "ยังไม่ได้เลือก" : "Nothing selected"}
                </p>
                <p className="text-[10px] text-slate-400 leading-relaxed">
                  {isTh ? "คลิกโต๊ะเพื่อดูรายละเอียดและแก้ไข" : "Click a desk to inspect and edit its properties"}
                </p>
              </div>
              <div className="mt-1 w-full space-y-1.5">
                <button onClick={() => setTool("add_seat")}
                  className="w-full flex items-center justify-center gap-1.5 text-xs text-indigo-600 bg-indigo-50/60 hover:bg-indigo-100/60 border border-indigo-200/60 rounded-xl py-2 font-medium transition-colors">
                  <Plus size={12} /> {isTh ? "เพิ่มโต๊ะใหม่" : "Add new desk"}
                </button>
                <button onClick={() => setTool("add_zone")}
                  className="w-full flex items-center justify-center gap-1.5 text-xs text-slate-600 bg-white/40 hover:bg-white/60 border border-slate-200/60 rounded-xl py-2 font-medium transition-colors">
                  <Square size={12} strokeWidth={1.5} /> {isTh ? "เพิ่มโซนใหม่" : "Add new zone"}
                </button>
              </div>
            </div>
          )}
      ...
      {/* ── Modals ── */}
      {editModal && (
        <SeatModal seat={editModal} isTh={isTh} employees={employees} assets={assets}
          ...
          onSave={s => {
            ...
            assetDB.getAll().then(allA => {
              oldTags.filter((t: string) => !newTags.includes(t)).forEach((t: string) => {
                ...
                if (a) assetDB.update(a.id, { seat_id: null, seat_label: null, room_name: null, status: a.status === "in_use" ? "idle" : a.status });
              });
              newTags.forEach((t: string) => {
                const a = allA.find(x => x.asset_tag === t);
                if (a) {
                  const newStatus = (a.status === "idle" || a.status === "in_use")
                    ? (hasEmployee ? "in_use" : "idle") : a.status;
              ...
              setSeats(p => p.map(x => x.id === s.id ? s : x));
              setSelected(s);
              setEditModal(null);
            });
          }}
  ...
  return (
    <button onClick={onClick} title={label}
      className={`flex items-center justify-center w-8 h-8 rounded-xl transition-all ${
        active ? "bg-white shadow-sm text-indigo-600" : "text-slate-500 hover:text-slate-700 hover:bg-white/50"
      }`}>
      {icon}
    </button>
  );
...
function Kbd({ children }: { children: React.ReactNode }) {
  return (
    <kbd className="inline-flex items-center px-1 py-0.5 bg-white/70 rounded border border-slate-200 text-[9px] font-mono text-slate-500 leading-none">
      {children}
    </kbd>
  );
}
...
function RHandle({ edge, onMouseDown, color = "#6366f1" }: {
  edge: ResizeEdge; onMouseDown: (e: React.MouseEvent) => void; color?: string;
}) {
    ...
    <MModal title={isTh ? `แก้ไขโต๊ะ ${seat.label}` : `Edit ${seat.label}`} onClose={onClose}>
          ...
          {(["vacant","occupied","maintenance","broken"] as SeatStatus[]).map(st => (
            <button key={st} onClick={() => setStatus(st)}
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border text-[11px] font-semibold transition-all ${
                status === st ? "shadow-sm" : "border-slate-200 text-slate-400 bg-white/50 hover:border-slate-300"
              }`}
              style={status === st ? { color: SA[st], background: SC[st], borderColor: SB[st] } : {}}
            >
              <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: status === st ? SA[st] : "#cbd5e1" }} />
              {SL[st][isTh ? "th" : "en"]}
            </button>
          ))}
        </div>
      ...
      <MF label={isTh ? "ทรัพย์สิน (เลือกได้หลายรายการ)" : "Assets (select multiple)"}>
        <div className="border border-slate-200 rounded-xl max-h-44 overflow-y-auto divide-y divide-slate-100">
          {assets.length === 0 && <p className="px-3 py-2 text-slate-400 text-[11px] italic">{isTh ? "ไม่มีข้อมูลทรัพย์สิน" : "No assets"}</p>}
          {assets.filter(a => a.status !== "returned" && (!assignedTags.has(a.asset_tag) || tags.includes(a.asset_tag))).map(a => {
            ...
            return (
              <label key={a.id} className={`flex items-center gap-2.5 px-3 py-2 cursor-pointer text-xs transition-colors ${checked ? "bg-indigo-50" : "hover:bg-slate-50"}`}>
          ...
          <div className="flex flex-wrap gap-1 mt-2">
            {tags.map(t => (
              <span key={t} className="inline-flex items-center gap-1 bg-indigo-100 text-indigo-700 rounded-full px-2 py-0.5 text-[11px]">
                {t}
                <button onClick={() => setTags(p => p.filter(x => x !== t))} className="hover:text-red-500 leading-none">x</button>
              </span>
            ))}
          </div>
        )}
      ...
      <MF label="RJ45">
        <input value={rj45} onChange={e => setRj45(e.target.value)} placeholder="P01" className={inp()} />
      </MF>
      <div className="flex gap-2 pt-1">
        ...
        <button onClick={handleSave} className="flex-1 bg-indigo-600 text-white rounded-xl py-2 text-sm font-medium hover:bg-indigo-700">
          {isTh ? "บันทึก" : "Save"}
        </button>
      </div>
    </MModal>
...
// ── Zone Modal ─────────────────────────────────────────────────────
function ZoneModal({ zone, isTh, onSave, onDelete, onClose }: {
  zone: Zone | null; isTh: boolean;
  onSave: (z: Zone) => void; onDelete: (id: string) => void; onClose: () => void;
}) {
  const [f, setF] = useState({
    label: zone?.label ?? (isTh ? "โซนใหม่" : "New Zone"),
    color: zone?.color ?? ZONE_COLORS[0],
    w: zone?.w ?? 200, h: zone?.h ?? 220,
  });
        ...
        <div className="grid grid-cols-5 gap-1.5">
          {ZONE_COLORS.map(c => (
            <button key={c} onClick={() => setF(p => ({ ...p, color: c }))}
              className={`h-9 rounded-xl border-2 transition-all ${f.color === c ? "scale-105 shadow-md" : "border-transparent hover:scale-105"}`}
              style={{ backgroundColor: c, borderColor: f.color === c ? (ZONE_BORDER[c] ?? "#94a3b8") : "transparent" }}
            />
          ))}
        </div>
      </MF>
      ...
      <div className="flex gap-2 pt-1">
        {zone && (
          <button onClick={async () => {
            const ok = await cfm({ title: isTh ? `ลบโซน ${zone.label}?` : `Delete zone ${zone.label}?`, confirmLabel: isTh ? "ลบ" : "Delete", danger: true });
            if (ok) onDelete(zone.id);
          }} className="flex items-center gap-1.5 border border-red-200 text-red-600 hover:bg-red-50 rounded-xl px-3 py-2 text-sm">
            <Trash2 size={13} />
          </button>
        )}
        ...
        <button onClick={async () => {
          const ok = await cfm({ title: isTh ? `บันทึกโซน ${f.label}?` : `Save zone ${f.label}?`, confirmLabel: isTh ? "บันทึก" : "Save" });
          if (ok) onSave({ id: zone?.id ?? uid(), x: zone?.x ?? 20, y: zone?.y ?? 20, ...f });
        }} className="flex-1 bg-indigo-600 text-white rounded-xl py-2 text-sm font-medium hover:bg-indigo-700">{isTh ? "บันทึก" : "Save"}</button>
      </div>
    </MModal>
  );
}
...
// ── Floor Modal ────────────────────────────────────────────────────
function FloorModal({ floor, isTh, onSave, onDelete, canDelete, onClose }: {
  floor: Floor | null; isTh: boolean; canDelete: boolean;
  onSave: (f: Floor) => void; onDelete: (id: string) => void; onClose: () => void;
}) {
  ...
  return (
    <MModal title={floor ? (isTh ? `แก้ไข ${floor.label}` : `Edit ${floor.label}`) : (isTh ? "เพิ่มพื้นที่" : "New Area")} onClose={onClose}>
      {CUI}
      <MF label={isTh ? "ชื่อพื้นที่" : "Area Name"}>
        <input value={label} onChange={e => setLabel(e.target.value)} autoFocus className={inp()} />
      </MF>
      <div className="flex gap-2 pt-1">
        {floor && canDelete && (
          <button onClick={async () => {
            const ok = await cfm({ title: isTh ? `ลบพื้นที่ ${floor.label}?` : `Delete area ${floor.label}?`, message: isTh ? "โต๊ะและโซนทั้งหมดจะถูกลบด้วย" : "All desks and zones will be removed.", confirmLabel: isTh ? "ลบ" : "Delete", danger: true });
            if (ok) onDelete(floor.id);
          }} className="flex items-center gap-1.5 border border-red-200 text-red-600 hover:bg-red-50 rounded-xl px-3 py-2 text-sm">
            <Trash2 size={13} />
          </button>
        )}
        <button onClick={onClose} className="flex-1 border border-slate-200 rounded-xl py-2 text-sm text-slate-600 hover:bg-slate-50">{isTh ? "ยกเลิก" : "Cancel"}</button>
        <button onClick={async () => {
          const ok = await cfm({ title: isTh ? `บันทึกพื้นที่ ${label}?` : `Save area ${label}?`, confirmLabel: isTh ? "บันทึก" : "Save" });
          if (ok) onSave({ id: floor?.id ?? uid(), label });
        }} className="flex-1 bg-indigo-600 text-white rounded-xl py-2 text-sm font-medium hover:bg-indigo-700">{isTh ? "บันทึก" : "Save"}</button>
      </div>
    </MModal>
  );
...
function MModal({ title, children, onClose }: { title: string; children: React.ReactNode; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={onClose}>
      <div
        className="w-full max-w-sm mx-4 p-5 space-y-4 max-h-[90vh] overflow-y-auto"
        style={{
          ...
          boxShadow: "0 24px 64px rgba(15,23,42,0.18), 0 0 0 1px rgba(255,255,255,0.5)",
        }}
